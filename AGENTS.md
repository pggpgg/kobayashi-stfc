...
- `**src/combat/**` — Core fight loop (`engine.rs`). This is the hot path: zero allocations, no dynamic dispatch, SplitMix64 PRNG. `abilities.rs` evaluates effects per round; `stacking.rs` and `effect_accumulator.rs` implement the stacking rules (per category, `base × (1 + Σpct) + Σflat`, then damage-channel multipliers and caps).
