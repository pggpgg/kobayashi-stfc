...
**Building opaque buff gaps:** `cargo run --bin report_building_mapping_gaps` (actionable-only Markdown; also `docs/building_gaps.md`). Allowlist: `data/buildings/opaque_buff_allowlist.json`. Baseline: `data/buildings/mapping_gaps_baseline.json`. Strict validate: `KOBAYASHI_REQUIRE_BUILDING_BONUS_MAPS=1` or `cargo run --bin validate_data -- --strict`.
