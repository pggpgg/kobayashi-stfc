...
/// Runs the per-weapon attack + counter-fire loop:
/// AttackPhase/DefensePhase effect filtering and activation logging; the `for weapon_index`
/// loop including outbound damage, crits, mitigation, shield/hull split, shield-break + kill
/// effects, counter-fire with AVX2 batch kernel, hull-breach state entry on both sides,
/// receive-damage rolls, and after-subround carry accumulation.
///
/// Persistent round-to-round state mutated through `&mut`. `phase_effects_round` is mutated
/// in place across all weapon sub-rounds.
...
fn run_attack_phase(
    ...
    num_sub_rounds: usize,
    b_shots: f64,
    def_b_shots: f64,
    attacker_mitigation_additive: f64,
    attacker_accuracy_bonus: f64,
    ...
    let defense_phase_filtered = filter_effects_by_condition(defense_phase_effects, combat_ctx);
    ...
    after_subround_carry.set_trace_contributions(pc.trace.is_enabled());
    for weapon_index in 0..num_sub_rounds {
        let mut defender_shield_break_carry: Vec<ActiveAbilityEffect> = Vec::new();
        phase_effects.clear();
        phase_effects.set_trace_contributions(pc.trace.is_enabled());
        ...
        let mut effective_pierce =
            attacker.weapon_pierce(weapon_index) + phase_effects_round.pre_attack_pierce_bonus();
        ...
        for hit_index in 0..effective_shots {
            if let Some(attacker_weapon_attack) = attacker.weapon_attack(weapon_index) {
                ...
                let roll = (pc.rng.next_u64() as f64) / (u64::MAX as f64);
                pc.trace.record_if(|| CombatEvent {
                ...
                let effective_mitigation = if let Some(params) = &defender.hostile_mitigation_params
                {
                    ...
                    mitigation_trace_breakdown = Some((breakdown, params.floor, params.ceiling));
                ...
                let (did_proc, proc_roll) = roll_weapon_intrinsic_proc(w_proc_chance, pc.rng);
                ...
                *total_shield_damage += actual_shield_damage;
                ...
                if hull_damage_this_round > 0.0 {
                    ...
                    let def_rd_filtered =
                        filter_effects_by_condition(defender_receive_damage_effects, &ctx_def_rd);
                    roll_burning_triggers(
                        pc,
                        &def_rd_filtered,
                        def_rd_assim,
        ...
        if let Some(defender_weapon_attack) = defender.weapon_attack(weapon_index) {
            ...
            for hit_index in 0..def_effective_shots {
                ...
                let defender_attack_phase_assimilated = *defender_assimilated_rounds_remaining > 0;
                ...
                defender_phase_effects.set_trace_contributions(pc.trace.is_enabled());
                ...
                let (def_did_proc, _def_proc_roll) = roll_weapon_intrinsic_proc(def_w_proc, pc.rng);
                            ...
                            *attacker_shield_gross_damage_this_round += att_actual_shield_damage;
                                    ...
                                    .chain(defender_defense_filtered.iter())
                                {
                                    ...
                                    } = effective_effect
                                    {
                                        ...
                                        let hull_breach_roll =
                                            (pc.rng.next_u64() as f64) / (u64::MAX as f64);
                                        let triggered = hull_breach_roll < chance.clamp(0.0, 1.0);
                                        ...
                                        if breach_before == 0 && *attacker_hull_breach_rounds > 0 {
                                            ...
                                            ctx_hb.attacker_hull_pct = 1.0
                                                ...
                                                .min(1.0);
                                            ctx_hb.attacker_shield_pct = if attacker.shield_health
                                                > 0.0
                                            {
                            ...
                            if attacker_own_shields_broke {
                                ...
                                ctx_self_sb.attacker_hull_pct = 1.0
                                    - (*total_attacker_hull_damage / attacker.hull_health.max(0.0))
                                        .min(1.0);
...
/// Values computed during the round-start phase that feed the attack phase and round-end.
/// Owned by `simulate_combat_from_setup`; built fresh each round inside [`run_round_start_phase`].
pub(crate) struct RoundStartOutput {
    pub combat_ctx: CombatContext,
    pub phase_effects: EffectAccumulator,
    pub b_shots: f64,
    pub def_b_shots: f64,
}

/// Runs the round-start phase: defender assimilate/regen, attacker context build, CombatBegin +
/// RoundStart accumulation, morale roll, hull-breach/burning/shots-bonus procs, prev-round heal,
/// and the AfterRoundStart snapshot. All persistent round-to-round state is mutated through `&mut`
/// parameters; transient outputs that feed the attack phase are returned in [`RoundStartOutput`].
#[allow(clippy::too_many_arguments)] // round-phase helper; explicit borrows over state struct
fn run_round_start_phase(
    pc: &mut RoundPhaseCtx<'_>,
    setup: &PreCombatSetup,
    total_hull_damage: &mut f64,
    total_attacker_hull_damage: &mut f64,
    ...
    for effect in &bench {
        ...
        {
            let shots_roll = (pc.rng.next_u64() as f64) / (u64::MAX as f64);
    ...
    let morale_triggered = {
        ...
        if let Some((morale_source, morale_chance)) = morale_source {
            let morale_roll = (pc.rng.next_u64() as f64) / (u64::MAX as f64);
