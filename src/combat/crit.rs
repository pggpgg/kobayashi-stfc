...
///
/// `additive_crit_damage_bonus` adds **percentage points** directly to the crit multiplier
/// (`weapon_crit_multiplier × crit_damage_multiplier + additive`), i.e. a `+X%` crit-damage stat
/// bonus rather than a multiplicative chain factor. Used by Rotarran "Bird of Prey" (per-crit
/// cumulative crit damage while the opponent is hull breached); `0.0` everywhere else.
...
mod tests {
    ...
    fn non_crit_yields_unit_multiplier() {
        ...
        let c = resolve_vehicle_weapon_crit(0.0, 0.0, 2.0, 1.5, 0.0, 0.0, 0.0, false, &mut rng);
        assert!(!c.is_crit);
    ...
    fn guaranteed_crit_scales_by_weapon_and_officer_crit_damage_mult() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.5, 0.0, 0.0, 0.0, false, &mut rng);
        assert!(c.is_crit);
    ...
    fn additive_crit_damage_bonus_adds_percentage_points_to_multiplier() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.0, 0.5, 0.0, 0.0, false, &mut rng);
        assert!(c.is_crit);
    ...
    fn additive_crit_damage_bonus_ignored_on_non_crit() {
        ...
        let c = resolve_vehicle_weapon_crit(0.0, 0.0, 2.0, 1.0, 0.5, 0.0, 0.0, false, &mut rng);
        assert!(!c.is_crit);
        assert!((c.multiplier - 1.0).abs() < 1e-12);
    }
    ...
    fn hull_breach_multiplies_crit_damage_bonus() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.0, 0.0, 0.0, 0.0, true, &mut rng);
        assert!(c.is_crit);
    ...
    fn same_seed_and_params_yields_identical_resolution() {
        ...
        let a = resolve_vehicle_weapon_crit(0.5, 0.1, 2.0, 1.2, 0.0, 0.0, 0.0, false, &mut rng_a);
        let b = resolve_vehicle_weapon_crit(0.5, 0.1, 2.0, 1.2, 0.0, 0.0, 0.0, false, &mut rng_b);
    ...
    fn effective_crit_chance_is_clamped_to_one() {
        ...
        let c = resolve_vehicle_weapon_crit(0.9, 0.5, 2.0, 1.0, 0.0, 0.0, 0.0, false, &mut rng);
    ...
    fn effective_crit_chance_is_clamped_to_zero() {
        ...
        let c = resolve_vehicle_weapon_crit(0.0, -0.5, 2.0, 1.0, 0.0, 0.0, 0.0, false, &mut rng);
        assert!(!c.is_crit);
    ...
    fn crit_chance_bonus_adds_to_weapon_crit_chance() {
        ...
        let c = resolve_vehicle_weapon_crit(0.25, 0.75, 2.0, 1.0, 0.0, 0.0, 0.0, false, &mut rng);
        assert!(c.is_crit);
        assert!((c.effective_crit_chance - 1.0).abs() < 1e-12);
    }
    ...
    fn attacker_crit_reduction_shrinks_base_multiplier_when_no_floor() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.0, 0.0, 0.50, 0.0, false, &mut rng);
        assert!(c.is_crit);
    ...
    fn crit_damage_floor_clamps_below_reduced_multiplier() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.0, 0.0, 0.80, 1.5, false, &mut rng);
        assert!(c.is_crit);
    ...
    fn crit_damage_floor_does_not_clamp_when_reduced_exceeds_floor() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.5, 1.0, 0.0, 0.20, 1.0, false, &mut rng);
        assert!(c.is_crit);
    ...
    fn floor_protects_against_hull_breach_compounding_on_clamped_base() {
        ...
        let c = resolve_vehicle_weapon_crit(1.0, 0.0, 2.0, 1.0, 0.0, 0.90, 1.0, true, &mut rng);
        assert!(c.is_crit);
    ...
    fn floor_is_a_noop_when_not_crit() {
        ...
        let c = resolve_vehicle_weapon_crit(0.0, 0.0, 2.0, 1.0, 0.0, 0.0, 5.0, false, &mut rng);
        assert!(!c.is_crit);
        assert!((c.multiplier - 1.0).abs() < 1e-12);
    }
}
