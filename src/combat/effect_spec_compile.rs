...
impl std::fmt::Display for EffectSpecCompileError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnsupportedTrigger(t) => write!(f, "unsupported trigger {t:?} for this compile path"),
            Self::UnsupportedModifierOperation { modifier, operation } => {
                write!(f, "unsupported modifier/operation pair: {modifier:?} + {operation:?}")
            }
...
pub fn compile_trigger(trigger: AbilityTriggerSpec) -> Result<TimingWindow, EffectSpecCompileError> {
    match trigger {
        AbilityTriggerSpec::CombatBegin | AbilityTriggerSpec::ShipLaunched => Ok(TimingWindow::CombatBegin),
...
pub fn compile_condition(spec: &AbilityConditionSpec) -> Result<AbilityCondition, EffectSpecCompileError> {
    match spec {
        ...
        AbilityConditionSpec::DefenderFactionIs { faction } => {
            let tag = OpponentFactionTag::from_data_slug(faction).ok_or_else(|| {
                EffectSpecCompileError::UnknownFactionSlug(faction.clone())
            })?;
        ...
        AbilityConditionSpec::Not { inner } => Ok(AbilityCondition::Not(Box::new(compile_condition(inner)?))),
...
pub fn compile_conditions_and(specs: &[AbilityConditionSpec]) -> Result<Option<AbilityCondition>, EffectSpecCompileError> {
    if specs.is_empty() {
        return Ok(None);
    }
...
pub fn compile_research_attack_effect(
    ...
    if operation != AbilityOperationSpec::Add {
        return Err(EffectSpecCompileError::UnsupportedModifierOperation { modifier, operation });
    }
    ...
    match modifier {
        ...
        _ => Err(EffectSpecCompileError::UnsupportedModifierOperation { modifier, operation }),
    }
}
...
pub fn compile_research_attack_phase_spec(spec: &CombatEffectSpec) -> Result<Ability, EffectSpecCompileError> {
    ...
    let effect = compile_research_attack_effect(spec.modifier, spec.operation, spec.value.as_ref().ok_or(EffectSpecCompileError::MissingScalarValue)?)?;
