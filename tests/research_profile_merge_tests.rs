...
/// Restores previous value on drop so parallel tests do not inherit `KOBAYASHI_*` overrides.
struct EnvVarGuard {
    key: &'static str,
...
#[test]
#[serial_test::serial]
fn research_derived_attack_phase_seats_disable_env_matches_burning_fixture() {
    let _env = EnvVarGuard::set("KOBAYASHI_COMBAT_EFFECT_SPEC_DISABLE", "1");
    let catalog = ResearchCatalog {
        source: None,
        last_updated: None,
        items: vec![ResearchRecord {
...
#[test]
#[serial_test::serial]
fn research_derived_attack_phase_seats_disable_env_matches_default_path() {
    let catalog = ResearchCatalog {
        source: None,
        last_updated: None,
        items: vec![ResearchRecord {
            rid: 365419691,
            name: Some("NS Burning parity".into()),
            data_version: None,
            source_note: None,
            levels: vec![ResearchLevel {
                level: 1,
    ...
    let default_path = research_derived_attack_phase_seats(&imported, &catalog);
    assert_eq!(with_disable, default_path);
}
