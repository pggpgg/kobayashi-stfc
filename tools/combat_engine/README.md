# Combat Engine (Phase 1)

## Phase 1.1 — Mitigation model
...
## Phase 1.2 — Effect stacking primitives

Implements strict-category stacking grouped by `(effect_kind, stat_key)` with canonical composition:
...
`EffectCategory` enforces legal categories (`base`, `modifier`, `flat`) and
`stack_effects(...)` returns per-group totals and the composed total.
