...
def transform(source_path: Path, output_path: Path, alias_path: Path, registry_path: Path, report_path: Path) -> None:
    ...
    validation_errors = validate(officers)
    ...
    for officer in officers:
        for field in REQUIRED_FIELDS:
            if field not in officer or officer[field] in (None, ""):
                errors.append({"id": officer.get("id"), "reason": f"missing required field '{field}'"})

        cid = officer.get("id")
        if cid in ids_seen:
            errors.append({"id": cid, "reason": "duplicate canonical id"})
        ids_seen.add(cid)

        abilities = officer.get("abilities", [])
        if not abilities:
            errors.append({"id": cid, "reason": "officer has no ability rows"})
