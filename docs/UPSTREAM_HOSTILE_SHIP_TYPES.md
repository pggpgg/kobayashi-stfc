...
The table below reflects `**data/hostiles/index.json`** at data version `**stfcspace-hostiles-2026-03-21`** (same distinct set appears in `data/upstream/data-stfc-space/summary-hostile.json` for this checkout).
...
| 1                    | 599                  | Armada target                                                        | `translations-navigation.json` key `armada_target_label` → **ARMADA TARGET**; hull `hull_type` 5 → survey in data; in `summary-hostile.json`, 55 rows with `ship_type` 1 set `is_outpost: true` (598 summary rows vs 599 index rows for this checkout). | `**ShipType::Armada`** via `is_armada_target`.      |
...
| 4                    | 112                  | Hunter-style hostiles                                                | Sample `loca_id` 14018 → **Klingon Hunter** (`officer_name`).                                                                                                                                                                                           | Hull-derived only.                                  |
...
| 7                    | 1,153                | System hostiles (hull `2` → **explorer** line)                       | Dominant upstream `hull_type` **2**; sample `loca_id` 60057 → **Romulan Velite** (`marauder_name_only`, hazard-tagged UI string).                                                                                                                       | Hull-derived only.                                  |
| 8                    | 434                  | Mixed hull lines (mostly hull `2`/`0`/`3`)                           | Mixed upstream `hull_type` in summary; sample `loca_id` 11003 → **Federation Patrol** (`marauder_name_only`).                                                                                                                                           | Hull-derived only.                                  |
...
| 13                   | 25                   | War coalition / mixed-line hostiles                                  | Mixed `hull_type`; sample `loca_id` 12005 → **Romulan War Coalition** (`marauder_name_only`).                                                                                                                                                           | Hull-derived only.                                  |
...
Human-readable triage report (includes this data as a section):
