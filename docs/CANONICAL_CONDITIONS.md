...
### After editing canonical officers

After `fetch_stfcspace_officers` / `normalize_officer_id_strings.py` or any hand-edit to `officers.canonical.json`:

1. `cargo run --bin report_unknown_mappings` — confirm **Still unmapped: 0**.
2. `cargo run --bin generate-lcars -- data/officers/officers.canonical.json --output data/officers` — confirm stderr has no `skipping unmapped canonical condition`.
3. Optional stricter CI / pre-merge: set `KOBAYASHI_REQUIRE_CANONICAL_CONDITION_MAPS=1` when running `validate_data` so any future unmapped token is an **error** (default remains **warning** only).