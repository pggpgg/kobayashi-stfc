...
- [ ] **Separate raw combat pipeline from CSV combat log parser**
  ...
  - [x] **Simulator-structured** sub-round state snapshots: [`CombatStateSnapshot`](../src/combat/snapshot.rs) / trace `state_snapshot` rows when `SimulationConfig.emit_state_snapshots` + `TraceMode::Events`; strict ingest rules for `schema_version` 3 ([`validate_canonical_timeline`](../src/combat/log_validate.rs), [`hydrate_ingested_state_snapshots_from_values`](../src/combat/log_ingest.rs)); documented in [`combat_log_format.md`](combat_log_format.md).
  - [ ] Preserve **client/toolbox-observed** subround-level events and full intermediate stat streams at the same fidelity as the live game (still depends on raw capture volume; optional `stats_snapshot` maps remain the unstructured escape hatch).
