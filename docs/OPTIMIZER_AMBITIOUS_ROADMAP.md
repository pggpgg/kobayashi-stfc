...
4. **The second committed case is on the wrong side of that cliff.** `enterprise_d_numeric` (`uss_enterprise_d` vs `2918121098`) scores 0.0 for every lane, every seed — zero regret and zero recall for tiered, genetic, warm-start, and the random control alike. It cannot distinguish any two methods, which is why the gate runs only `saladin_corvus`. A discriminating second case still needs choosing.

Still open on this item: method-specific subphase timings (lane records carry total elapsed plus tiered scout/confirm trial counts, not a per-phase breakdown); the diversity metrics are computed per run but not yet aggregated across the seed panel; and the harness ships two cases on one profile — and per finding 4 only one of them measures anything — so it cannot yet show that a change generalizes across matchups.
