...
Four findings came out of building it and running it since:

1. **The committed `saladin_numeric` case was measuring nothing.** `saladin` is not a ship id (`uss_saladin` is), and an unresolved ship or hostile id did not fail — it produced a fight the crew wins in round 1, every time, at any hostile level. The case is replaced by `saladin_corvus` (`uss_saladin` vs `1140710508`, a matchup where the ranking score actually separates crews) and the harness now refuses a case whose ids do not resolve. **The silent fallback is fixed** — unresolvable ship and hostile ids, and out-of-range ship tiers, are now rejected at every API and CLI ingress rather than falling back to a synthesized fight.
2. **Stratified random beats tiered on this case at equal budget** — 0.0 mean score regret against tiered's ~0.0053 — and the cause is the size of the proposal space, not how it is truncated. Two separate narrowings were in play:
   - **Capped truncation covered one captain.** `exhaustive_candidates` and `sampled_candidates` walked captain-major and stopped at `max_candidates`, so the default budget of 128 was spent inside the first (captain, bridge pair) cell. Measured on the demo profile against pools of 189 captains / 242 bridge / 51 below-decks, the generator returned crews for **exactly one captain** at every seed. Truncation is now stratified — cells are visited bridge-pair-major and captain-minor, one crew per cell per pass, with each cell's starting below-decks combination staggered — so a budget reaches every captain the pool limits allow. Score regret fell to 0.00532 (tiered) and 0.00298 (warm_start_tiered).
   - **The pool limits are what remains.** `large_pool_captain_limit` / `large_pool_bridge_limit` still cut 189 captains to 10 and 242 bridge officers to 12, leaving a proposal space of 638–660 crews on this case. The reference sweep covers all of it — its cap does not bind — and its best crew scores 0.9983, while random_stratified reaches 1.0 on all five seeds by sampling the full pools. Raising the limits to 32/32 was measured and made tiered's regret **worse**: at a fixed candidate budget a wider pool just spreads the same 128 crews thinner. Closing the gap means widening the proposal space and the candidate budget together, which is §2.4 racing work, not a knob change.
...
4. **The second committed case was on the wrong side of that cliff — replaced 2026-08-15.** `enterprise_d_numeric` (`uss_enterprise_d` vs `2918121098`, a level 81 battleship with 2.2e15 hull) scored 0.0 for every lane, every seed — zero regret and zero recall for tiered, genetic, warm-start, and the random control alike, because every crew loses identically. It is replaced by **`nx01_l58_interceptor`** (`enterprise_nx_01` vs `1120713907`, a level 58 interceptor), and the gate now runs **both** cases.

   Selecting it surfaced a second degenerate mode, distinct from the unwinnable one. A matchup sitting *on* the win/lose cliff looks discriminating — regret of 0.4 to 1.0 — but that number is driven entirely by which side of the cliff each seed landed on (`best_win_rate` stddev near 0.5), so a gate built on it fires on noise. Candidates were rejected on exactly that basis: `vi_dar_talios` vs `1012429023`, `enterprise_nx_01` vs `1663855787`, `uss_voyager` vs `1120713907`, and two `monaveen` explorer matchups. A third mode is the mirror of the first: `gorn_eviscerator` vs `1665329672` is comfortably winnable and every lane scores exactly 0.0 regret.

   So a usable case needs both halves — the ranking score must separate lanes **and** every lane must win on every seed. `nx01_l58_interceptor` holds all four lanes at win rate 1.000 ± 0.000 while separating them (tiered 0.0079, warm-start 0.0076, genetic 0.0038, control 0.0), with tiered recall at 0.30 so the recall floor has room to detect a drop. It complements `saladin_corvus` by hostile class (interceptor vs battleship) and by ship.

   The gate's baseline schema went to **v2** for this: `config.cases` replaces `config.case`, and `methods` is keyed by case, since numbers from different matchups are never comparable and must not be pooled. Verified that cases do not perturb one another — `saladin_corvus` run alone reproduces its multi-case numbers exactly.

   One finding fell out of the rebaseline: `saladin_corvus` had drifted (tiered recall 0.22 → 0.24, regret 0.005324 → 0.005376) since its numbers were last written. Five commits touching `src/combat`, `src/optimizer`, or `src/data` landed in between — `a167441e`, `e0e9240c`, `4a90a205` (confidence-aware scout racing), `62ba8451` (hostile ability values from the upstream level curve), `f5c33e0a` — and **which one moved it was not pinned down**; scout racing and the ability-curve fix are the plausible candidates, since the first changes tiered scouting directly and the second changes fight behavior for a hostile that has abilities. The drift is not an artifact of the multi-case run: `saladin_corvus` alone reproduces the new numbers exactly.

   The lesson is the process one, and it holds whichever commit was responsible. Both moves sat inside the committed tolerances, so the gate stayed green while its baseline quietly went stale. **A behavior change — combat or optimizer — should be followed by a deliberate rebaseline**, not left to drift until a tolerance happens to trip.

Still open on this item: method-specific subphase timings (lane records carry total elapsed plus tiered scout/confirm trial counts, not a per-phase breakdown); the diversity metrics are computed per run but not yet aggregated across the seed panel; and both gated cases still run against the **same demo profile**, so the harness shows that a change generalizes across matchups but not yet across rosters.

**Recall@10 is a set-overlap metric and reads diversity as regression.** Rebaselining the stratified-truncation change dropped tiered's recall from 0.36 to 0.22 against an unchanged reference set. On seed 11 recall fell 0.8 → 0.1 while regret stayed at ~3e-5: the lane found an equally good crew that was not one of the reference's particular ten. Read recall together with regret, and prefer regret when they disagree.
