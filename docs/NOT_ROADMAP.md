...
- **Scoped combat building buffs** — Actionable opaque backlog is cleared (see `docs/building_gaps.md`). Allowlisted rows include armada-fleet participant weapon damage and ACAD crit mitigation (no engine stat). Aggregation hyperthermic stabilizer (`buff_1422729787` / Recon Locus) is modeled vs Aggregation hostiles. Station-defense mode remains future work.
