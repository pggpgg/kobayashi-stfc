...
- [ ] **2. Optimize job store robustness** — Harden `Mutex` usage in `src/server/api/execution.rs` (async optimize jobs); avoid assumptions that locks never fail.
...
- [ ] **13. Static asset compression and caching** — Compression for `frontend/dist`; cache-control for hashed Vite assets.
...
- [ ] **17. Supply-chain checks** — `cargo audit` / `npm audit` in CI with an explicit fail vs warn policy.
