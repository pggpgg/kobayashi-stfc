...
- **Sobol variance decomposition** (`/sensitivity` method toggle, `POST /api/sensitivity/sobol`, CLI `sobol-sensitivity`): Saltelli sample design with Jansen-1999 estimators. Reports **S_i** (first-order index — fraction of Var(Y) from stat `i` alone), **S_T_i** (total-order — main + all interactions involving `i`), and **interaction strength** `S_T_i − S_i` per stat. Compute is `N × (k + 2)` engine calls; defaults `N=512`, `k=18` sum to ~10k sims (cheaper than Morris/OAT defaults). Bootstrap 95% CIs on S_i / S_T_i (resampled rows, no extra engine calls). CRN pairing across A and A_B^(i) rows reduces estimator variance. Per-pair S_ij (which two stats specifically interact) is planned as a v2 follow-up (`N × k(k−1)/2` extra evaluations).

## Planned

- Synergy learning from simulation results (co-occurrence matrix → bias future searches)
- Armada mode (multi-ship combat)
- **Sobol pairwise indices (S_ij)** — extend the shipped first-order / total-order Sobol with per-pair second-order indices to surface specific interacting stat pairs (`armor × accuracy`, `crit_chance × crit_damage`, …). Cost: an additional `N × k(k−1)/2` evaluations on top of the existing `N × (k+2)` baseline. Build on [src/optimizer/sensitivity_sobol.rs](../src/optimizer/sensitivity_sobol.rs).
...
- **`player_crit_damage_reduction` in sensitivity.** Currently plumbed through a PvP-specific crew-extension mechanism ([src/optimizer/monte_carlo/scenario.rs:205](../src/optimizer/monte_carlo/scenario.rs)) — there's no post-resolution scalar to perturb. v1 ships a universal `crit_damage_reduction` perturbation that rides on top of the resolved crew-derived reduction via `Combatant.crit_damage_reduction_bonus` ([src/combat/engine.rs](../src/combat/engine.rs)); making the player-side path uniform with the hostile-side path would require a new additive-effect variant the seat-walk sums separately (see PR notes on the max-vs-additive semantic). Deferred — current modeling produces correct per-round sums.
- **Per-round additive stacking for sibling resolvers.** Two crew-walk resolvers in [src/combat/abilities.rs](../src/combat/abilities.rs) still use the old `max`-aggregation pattern despite the in-game effects stacking additively: `hostile_counter_stat_debuff_from_crew` (line 848, used for Quv'Sompek / B'Rel pierce-counter-fire debuffs) and `defender_shield_drain_per_round_from_crew` (line 875, used for Sanctus-style per-round shield drain). Both should follow the per-round-sum pattern that [`hostile_crit_damage_reduction_active_at_round`](../src/combat/abilities.rs) uses, gating each seat by its own `duration_rounds` window. Reference [`sum_hostile_engagement_defensive_bonus`](../src/combat/abilities.rs) as the existing precedent for summed aggregation. Like the CDR fix, this is most visible for crews running multiple overlapping sources.
