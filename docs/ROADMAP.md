...
### Implemented (below-decks seating + `AddState` → LCARS)

- **Wrong `slot` vs upstream:** For several VOY officers, stfc.space JSON places a combat proc on **`below_decks_ability`** while canonical had **`slot`: `officer`**, so [`generate_lcars`](../src/bin/generate_lcars.rs) merged those effects into **`bridge_ability`** and left **`below_decks_ability: null`**. [`resolve_crew_to_buff_set`](../src/lcars/resolver.rs) only applies **`below_decks_ability`** when the officer is in a below-decks seat — so the proc never ran from the intended slot.
- **Canonical `AddState` attributes:** Rows use **`state=8` / `state=4` / `state=2` / `state=64`** (e.g. `num_rounds=1, state=8`). The generator previously looked for substrings like **`state8`**, so it fell through to a non-combat **`tag`** that [`resolve_effect`](../src/lcars/resolver.rs) skips. **Fix in code:** [`add_state_type_from_attributes`](../src/bin/generate_lcars.rs) parses the numeric id after `state=` (avoids e.g. `state=20` matching burning). **Fix in data:** canonical **`slot`: `below_decks`** where upstream says below-decks; then `cargo run --bin generate_lcars` and refresh **[`data/officers/officers.lcars.yaml`](../data/officers/officers.lcars.yaml)** (or merge from per-faction outputs).
- **Officers corrected (canonical + monolith):** Any ability whose **`ability_id`** equals upstream **`below_decks_ability.id`** must use **`slot`: `below_decks`** so [`generate_lcars`](../src/bin/generate_lcars.rs) emits a real **`below_decks_ability`** block. A repo-wide pass fixed **72** officers that had that id under **`slot`: `officer`** (including **Harry Kim**, **B’Elanna Torres**, **Neelix**, Lower Decks / WOK / SNW / etc.). **Maintenance:** re-run after upstream officer re-imports with [`scripts/sync_canonical_below_decks_slots.py`](../scripts/sync_canonical_below_decks_slots.py), then `cargo run --bin generate_lcars --release` and `cargo run --bin merge_lcars --release` (move aside the existing monolith first so merge does not load duplicate officer lists).
