...
- **Product polish & speed (web UI)** — The next main goal. The engine, data, and optimizer are mature (officer modeling fidelity ~maxed, search well-tuned, component upgrades shipped); focus is refining the React SPA. The app is solid at desktop width — incremental refinement, not a rebuild.
  ...
  **Remaining (optional, larger increments):** the headline inline-style consolidation is done; what's left is bigger and discretionary.
  - **Shared `ResultsTable<T>`** — SimResults / SobolResults / MorrisResults render near-identical paginated tables (~200 lines of overlap); a generic table component would de-dupe them. The per-view `styles` consts from #239 are the natural seed.
