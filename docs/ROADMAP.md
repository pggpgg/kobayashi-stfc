...
- **High priority mission:** **Battlelogs** — accept, persist, and wire mod **battlelogs** payloads into Kobayashi (calibration, recorded-fight fixtures, replay or analysis tooling; schema and storage path TBD alongside [SYNC.md](SYNC.md) / `src/server/sync.rs`).
