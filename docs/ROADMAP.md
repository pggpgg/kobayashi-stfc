...
- **Tiered below-decks filtering (not just binary):** Move from `only_below_decks_with_ability: bool` to three tiers: **(1) strict** (combat modifier only, current default), **(2) scored** (all below-decks-ability officers ranked by combat-relevance score: combat modifiers first, then ambiguous/missing modifiers, then economy-only, with officer power as a secondary tiebreaker), **(3) relaxed** (all officers regardless of ability, sorted by power — current `allow_below_decks_without_combat_ability` behavior). The "scored" middle tier gives the optimizer officers with unknown or marginal abilities a chance without fully opening the floodgates.
