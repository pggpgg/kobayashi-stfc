...
- **Sobol sensitivity** — variance-decomposition for first-order, total-order, and pairwise interactions. Answers "what is the best **pair** of stats to invest in together." Sobol (~`N × (2k+2)` sims at `N` ≥ 1024) for stable indices. Build on top of the v1 OAT engine in [src/optimizer/sensitivity.rs](../src/optimizer/sensitivity.rs) and the Morris elementary-effect runner in [src/optimizer/sensitivity_morris.rs](../src/optimizer/sensitivity_morris.rs). (Morris screening shipped — see Shipped.)
