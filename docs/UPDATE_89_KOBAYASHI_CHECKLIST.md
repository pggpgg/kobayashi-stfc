...
- [x] **Crew nullification**: Kathryn Janeway, Enterprise-E Picard, Christopher Pike have **no effect** vs hostiles tagged `crew_nullification` (Conqueror Borg + V'Ger-line `faction.loca_id` 86001); LCARS resolve + legacy `officer_id` seats
- [x] Officer **Mara Dalen** in LCARS: **Right to Protest** (Isolytic Defense vs Group Armadas); **Defy Defeat** (round-start shield repair vs all Armadas)
...
- [ ] Tests: nullified captains vs tagged hostiles; Mara round-start shield repair (done); optional calibration if fixtures exist
...
*Last updated: 2026-04-19 — Crew nullification implemented (`crew_nullification` tag, Janeway / Ent-E Picard / Pike); Borg Sphere passives + `conqueror_borg` tagging complete where implemented; beams / Evolutionary Assimilation still open.*
