...
| Defender fire delay | LCARS `allreloadspeed:enemy_delay` / `AllLoadSpeed` on enemy + Add; `DefenderFireDelay` skips defender counter-fire at combat_begin, round_start, attack (crit-gated), shield_break | **implemented** (per-weapon reload nuance not modeled; delay skips all counter-fire regardless of weapon type) |
| Attacker weapon recharge | LCARS `allreloadspeed:self_recharge` / self + Sub at combat start; proxied as round-1 `ShotsBonus` (+100% shots) | **partial** (recharge ≈ double shots hypothesis; `ModuleKinetic` gate honored — bonus applies to kinetic weapons only, untyped weapons match leniently) |
| Weapon-type gating | `ModuleKinetic` / `ModuleEnergy` conditions → `Ability.weapon_scope`; `WeaponStats.weapon_type` from `ships_extended` / hostile components (upstream `weapon_type`: 1 = Energy, 2 = Kinetic). Scoped `ShotsBonus` and CombatBegin/RoundStart `AttackMultiplier` / `CritDamageMultiplier` apply per matching weapon; untyped weapons match every scope | **implemented** (attacker side; defender counter path stays lenient. `HitEnemyWithKinetic` / `HitEnemyWithEnergy` trigger-conditions remain `literal_false`) |
| Once-per-weapon on-hit stacks | Seska-shaped `CritDamage` + `EnemyTakesHit` + `num_rounds` → `OnHitCritDamageStack`: one stack per weapon, refreshed when that weapon hits; stacks multiply as `(1+X)^active` | **implemented** (refresh-on-hit reading; once-per-fight alternate reading agrees for fights ≤ duration) |
...
2. **Normalizer:** `cargo run --bin normalize_data_stfc_space` reads `data/upstream/data-stfc-space/ships/<numeric_id>.json`, applies the catalog, and writes [data/ships_extended/](../data/ships_extended/) `<kobayashi_id>.json` with optional top-level `abilities`: `{ id, timing, effect_type, value, level_scaled_values? }`. `value` is always the `values[0]`-derived scalar (or the catalog `value_override`); when the ability's live upstream `values[]` entries vary, the normalizer also emits the per-level curve as `level_scaled_values` (auto-detected; `values_scale_with_ship_level` force-emits). Curves are suppressed for `combat_noop` rows (no effect to scale) and for rows with a `value_override` (a curve would overwrite the override at resolution), and are truncated to the ship's max level (upstream arrays are zero-padded to a fixed width, and some keep rising past the level cap).
3. **Load / resolve:** [src/data/ship.rs](../src/data/ship.rs) (`ExtendedShipRecord`, `ShipRecord`) deserializes `abilities`; `ExtendedShipRecord::to_ship_record` copies them onto the flat record for the chosen tier/level, collapsing `level_scaled_values` into a concrete `value` via `ship_ability_value_for_level` (index = **level − 1**; out-of-range levels fall back to the last positive entry, i.e. the max-level value).
...
**Implemented (example):** U.S.S. Crozier *Gunboat Diplomacy* maps to `hostile_crit_damage_reduction` in the catalog: on **defender return fire**, when the hostile rolls a crit, damage uses `crit_mult = max(floor, crit_mult × max(0.05, 1.0 - reduction))` for combat rounds `1..=duration_rounds` (default 5), with hull-breach amplification applied after the floor. The full-multiplier convention is supported by the official [Critical Mitigation guide](https://startrekfleetcommand.com/news/starfleet-academy-remote-campus-critical-mitigation/), whose worked example applies 62.41% mitigation to the full critical damage rather than only the bonus above ×1.0. Xindi additive-points crit reduction remains a separate bonus-only stat family.
...
- **Defender hull breach + cumulative weapon damage** — `round_start` + `accumulating_attack_multiplier` with `condition_defender_hull_breach` (D4 / Krennla-style lines).
- **Defender hull breach + cumulative crit (proc chains)** — `attack_phase` + `cumulative_breach_crit_chance` (Hegh'ta "Open the Wound", per **hit**) or `cumulative_breach_crit_damage` (Rotarran "Bird of Prey", per **crit**). Applied per shot at the crit-resolution site while the opponent is hull breached: crit chance grows additively (`+ per_hit × prior breached hits`, the roll clamps to `[0,1]`); crit damage grows as **additive percentage points** on the crit multiplier (`crit_mult = weapon_crit × officer_crit + per_crit × prior breached crits`, via a dedicated additive arg to `resolve_vehicle_weapon_crit`). Both uncapped (snowball), bonus reflects prior events only. See [SHIP_ABILITY_COMBAT_NOOP_AUDIT.md §6.5](SHIP_ABILITY_COMBAT_NOOP_AUDIT.md). Other hull-breach + crit chains whose text requires further chained procs remain `combat_noop`.
...
- **Isolytic offense / defense** — `combat_begin` + `isolytic_damage` or `isolytic_defense`. Value-scale conventions ground-truthed 2026-07-16 (backlog #13, `tests/isolytic_value_scale.rs`): C#-style `%` placeholders (`{0:0.#%}`, `{1:#.#%}`) render the upstream number ×100, so the upstream value is already an engine-unit **fraction** and is never divided by 100 — regardless of the upstream `value_is_percentage` flag (Something To Prove: 68.1 → +6,810%; the old flag-driven ÷100 was 100× too small). Multi-stat texts reuse `values[0].chance` as the second placeholder `{1}` (catalog `value_source: "chance"`; Double Down: `value` = flat apex barrier, `chance` = isolytic defense fraction). Hardcoded-number texts pin `value_override` from the text (Isolytic Dampeners 1000% → 10.0; Interdimensional Threat family). Conditional self-debuffs ("Isolytic Defense is reduced by X% when fighting a battleship/explorer") emit negative seats gated by `condition_attacker_ship_type` → `AttackerShipTypeIs`. Only no-placeholder, no-hardcoded-number multi-stat texts (Black Market Armaments, Krenim Temporal Core, Static Displacer/Collider) keep the legacy flag-driven scale — unvalidated, documented in the audit doc; Replicated Honorguard Apex is a review-bucket noop (per-stat values unattributable).
- **Apex shred / barrier** — `combat_begin` + `apex_shred` / `apex_barrier`. Both are directional (2026-07-15 rewiring): attacker-crew `apex_shred` boosts the player's outbound apex factor, while attacker-crew `apex_barrier` is **defensive only** — condition-filtered seats raise the barrier term of the **counter-fire** apex factor (`attacker_apex_barrier_bonus_active`, `src/combat/abilities.rs`) and never shield the defender from the player's own fire. PvP-defender barrier seats fold into the defender stat at scenario build unconditionally (seat conditions are not evaluated there — documented approximation).
- **Critical Mitigation rating (e.g. U.S.S. Athena "Athena's Revenge")** — `combat_begin` + `crit_mitigation_rating`: a flat rating converted at resolution to the existing counter-fire crit-damage-reduction path via `reduction = CM / (CM + 50,000)`, always active. The formula is pinned by the official Remote Campus guide's worked example (rating 83,000 ⇒ 62.41% off the **full** critical damage); the resolver's 0.95 reduction clamp binds above rating 950,000.
...
- **Combat start: armor/shield piercing or weapon damage** — Mapped to `combat_begin` + `pierce_bonus` or `attack_multiplier` with percentage flags set from text heuristics. **“Ignores X% of … shields” (2026-07-17 reclassification)** — Mapped to `shield_mitigation_bypass` (`AbilityEffect::ShieldMitigationBypassFraction`, the Harrison Sabotage outbound path), not pierce: bypass scales the target's shield mitigation and is inert against a shieldless target, while pierce is a fraction-scale additive damage-through term that would help everywhere. Sole live instance: U.S.S. Vengeance **Advanced Sabotage** (`2432056626`, upstream value 1 with a `{0:#.#%}` placeholder → 100% bypass), gated on the `breen_warship` hostile tag (Breen Warship / Elite / Primarch, `loca_id` 80600–80602). See `tests/vengeance_advanced_sabotage.rs`.
- **Upstream `values[]`** — The live (non-zero-tail) portion is a **per-level** curve: its length matches the ship's `max_level` (never the tier count — e.g. Enterprise-D: 60 live entries, 12 tiers; Augur: 45 live entries, 9 tiers), so index = level − 1. The normalizer emits `level_scaled_values` for every varying curve (truncated to max level) and the resolved ability magnitude scales with the requested ship level; constant curves stay a scalar `value`.
...
5. **Combat loop:** Counter-fire reads defender `CombatBegin` / round effects from `defender_crew` (isolytic, apex barrier, proc multipliers, etc.) via the same effect accumulator as ship hull abilities. **Critical Training / Diverted Power (2026-07-07):** Critical Training (`2291206649`) emits `crit_chance` plus `crit_damage` and `hostile_crit_damage_floor` extra seats; Diverted Power (`2486538514`, `788454016`) emits `hostile_crit_damage_floor`. Counter-fire crit floors clamp after player hostile-crit-damage reduction and before hull-breach amplification, matching the outbound reduction → floor → hull-breach ordering. **Xindi (2026-06-16):** round-start `hostile_crit_damage_reduction` subtracts upstream magnitude in percentage points from the player's outbound crit *bonus* (value 25 → 2500% UI → −25 points, collapsing typical crit toward ×1.0 before Critical Damage Floor). Stacking seats sum reduction first. **`1271329828` (Doomed Species + Xindi Weaponry):** round-end `hostile_lethal_end_of_round` instant kill every round (particle beam; separate from the 6×100k weapon component). **`141924765` (Denticle Blade):** combat-start 30% proc gates counter-fire for weapon slot 5 (heavy artillery component stats). **Be Like Water / Xindi Might** (`1408273502`, `141924765`): Xindi Might lethal text is the 9×20B weapon only — no catalog lethal seat. **No Mercy** armadas: lethal every 8th round; assimilated = 100% prevent. **Defender-alive gate (2026-07-27):** every `hostile_lethal_end_of_round` seat is skipped when the hostile's post-round-end HHP is already ≤ 0 — the round loop resolves round end *before* its death check, so without the gate a kill landing on the interval round (Species 8472 at round 8, Q Trials Borg Defense Protocol α at every round) fired the dead hostile's beam and turned the win into a mutual-death loss. **Kemocite:** round-end +30% stack when not burning. **Fidelity pass (2026-07-08):** upstream `values[].chance: 1` now normalizes to 100% (was 1% — all 22 live proc-typed rows were effectively inert); `ProcAttackMultiplier` composes as `×(1 + value)` so catalog values are bonus fractions matching upstream texts; catalog entries accept `round_cap` ("for the first N rounds" → `RoundRange` seat condition); defender combat-begin `burning` seats roll onto the **player's** burning state (Persistence Hunter, 1% max-hull tick/round); `hostile_counter_pierce_multiplier` scales counter-fire pierce ×(1+X) (Pen of Kahless — pierce is fraction-scale in the additive damage-through factor, so a flat `pierce_bonus` would be inert); Psionic Assault maps to `round_start` `hostile_hyperthermic_decay`. Generator treats `{0:#.#%}` placeholder values as fractions (no ÷100). **Faction-gated lethal strikes (2026-07-08):** Tal Shiar / Mo'Kai / S31 Elite and Q Almost Omnipotent / Strike Down (`hostile_lethal_unless_attacker_faction`) instant-lose wrong hull-design factions at combat start (same early-return as Conqueror Borg beams); Q texts allow `uss_vengeance` by ship id; both Q abilities set `hostile_crit_damage_floor` 3.0; Strike Down adds `hostile_attacker_shield_mitigation_zero` (incoming SM forced to 0%). **Per-hit stacks (2026-07-08):** Critical Breach / Rising Fire (`DefenderOnHitStack`) push uncapped per-hit stacks lasting 2 rounds while the player is hull-breached / burning; stacks earned on hit N boost hit N+1 same round ("prior events only"); Critical Breach also sets crit floor 1.5. Companions Hole Puncher / Immolator apply combat-begin `hull_breach` / `burning` to the player for the rest of combat (`MAX_COMBAT_ROUNDS`). **Dilithium Destabilization (2026-07-08):** `hostile_lethal_combat_begin` rolls once per trial at combat begin from upstream `values[].chance` (0.9 / 0.3); on success, same instant-loss early return (`rounds_simulated == 0`); RNG only when a seat exists (after Denticle / Burning / HullBreach combat-begin rolls). **Energy-Dampening Field (2026-07-19):** Breen Warships (`hostile_shield_damage_routing`, 8 hostiles) route ALL outbound damage into the shield pool — the per-shot split, the round-end bonus/burning terms, and combat-end damage all use effective shield mitigation 1.0, with overflow past remaining SHP spilling to hull — and regenerate 25% of max SHP at the start of each round (clamped, no RNG). Resolved out of band at setup (`defender_shield_damage_routing_regen`). The per-hit officer/FT `ShieldMitigationBypassFraction` channel is ignored while routing is active ("cannot be altered by officers, Forbidden Tech"); only tag-gated `CrewSeat::Ship` bypass seats (Vengeance Advanced Sabotage vs `breen_warship`) fold into the routing override at setup as the designed counter. Approximations: burning/round-end damage rides the shield pool rather than ticking hull (so burn crews are under-credited when a counter keeps shields up), and the counter bypass applies only to weapon fire. See [HOSTILE_ABILITY_COMBAT_NOOP_AUDIT.md](HOSTILE_ABILITY_COMBAT_NOOP_AUDIT.md) and `tests/hostile_fidelity_new_mechanics.rs`.

**Shield Disruptors (2026-07-19):** loca 55049 resolves to `defender_on_hit_shield_mitigation_reduction` for all 31 carrier ids. A hostile weapon hit pushes a 0.10 additive-point reduction to the player's shield-mitigation fraction after that hit resolves; later hits in the same round see all prior stacks. The stacks use `DefenderOnHitStack` with the ungated `Always` trigger and `duration_rounds: 1`, so they are pruned before the next round. Shield mitigation is clamped to `[0, 1]` after subtracting stacks and before applying hostile bypass.
...
**Jansen 1999 estimators.** For each stat `i`:

```text
V(Y)   ≈ Var(f(A) ∪ f(B))                           (combined-sample estimator)
V_i    ≈ (1/N) Σ f(B)_j · (f(A_B^(i))_j − f(A)_j)
V_T_i  ≈ (1/2N) Σ (f(A)_j − f(A_B^(i))_j)²
S_i    = V_i / V(Y)         (clamped to [0, ∞) for display)
S_T_i  = V_T_i / V(Y)        (lower-bounded at 0)
```

The Jansen total-order estimator is the recommended robust choice — positive by construction and well-behaved when V(Y) is small.
...
```
1. Edit LCARS under `data/officers/` (`officers.lcars.yaml`, or any `*.lcars.yaml` / `*.lcars.yml` if using a multi-file layout). Regenerate the monolith from canonical with `generate_lcars`.
