...
5. **Combat loop:** Counter-fire reads defender `CombatBegin` / round effects from `defender_crew` (isolytic, apex barrier, proc multipliers, etc.) via the same effect accumulator as ship hull abilities. **Xindi (2026-06-16):** round-start `hostile_crit_damage_reduction` stacks up to `duration_rounds` overlapping copies (`hostile_crit_damage_reduction_active_at_round`); round-end `hostile_lethal_end_of_round` zeros attacker remaining hull on schedule (`round_interval`, default every round). Lethal is modeled as unmitigated hull loss (shot count is metadata only). **Kemocite (`3981152012`, six group armadas):** round-end +30% counter-fire stack when the hostile is not burning (burning = 100% prevent); stacks are uncapped (`defender_kemocite_stacks`). **No Mercy (`2665723295`, same armadas):** lethal on every 8th round; assimilated on the hostile prevents 100%.
