...
const { apiMocks, doneOptimizeStatus } = vi.hoisted(() => {
  ...
  const apiMocks = {
    ...
    mockOptimizeStart: vi.fn().mockRejectedValue(new Error('skip optimize in test')),
...
describe('useWorkspace', () => {
  ...
  it('resumes persisted job: fetches status and clears storage when job is already done', async () => {
    ...
    await waitFor(() => {
      expect(apiMocks.mockGetOptimizeStatus).toHaveBeenCalledWith('job-resume-1');
    });
    expect(sessionStorage.getItem('kobayashi_active_optimize_job_v1')).toBeNull();
